...
def test_infinite_scroll(page: Page):
    ...
    # Get initial count of elements loaded dynamically
    initial_count = page.locator('.jscroll-added').count()
    print(f'Initial Count : {initial_count}')
    
    # Scroll 5 times
    for i in range(5):
        # Playwright equivalent to window.scrollTo() via page.evaluate
        page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
        
        # A tiny pause giving the API a moment to load and append new content
...
def test_large_and_deep_dom(page: Page):
    ...
    # Assert specific sibling elements are present/visible
    expect(page.locator('#sibling-1\.1')).to_be_visible()
    
    # Scroll to the bottom of the page
    page.evaluate("window.scrollTo(0, document.body.scrollHeight)")
    
    expect(page.locator('#sibling-50\.3')).to_be_attached()
